import org.junit.jupiter.api.*;
import static org.junit.jupiter.api.Assertions.*;
import java.util.List;

public class NotificationTest {

    private MockNotificationService mockService;
    private NotificationManager notificationManager;

    @BeforeEach
    void setup() {
        mockService = new MockNotificationService();
        notificationManager = new NotificationManager(mockService);
    }

    // ── sendBookingConfirmation ───────────────────────────────────────────────

    @Test
    void shouldSendBookingConfirmation() {
        String email = "test@example.com";
        String details = "Appointment on 2025-01-15 at 10:00";

        notificationManager.sendBookingConfirmation(email, details);

        List<String> messages = mockService.getSentMessages();
        assertEquals(1, messages.size());
        assertTrue(messages.get(0).contains("CONFIRMATION"));
        assertTrue(messages.get(0).contains(email));
        // NotificationManager wraps details inside its message — the original
        // details string is embedded verbatim by createBookingConfirmationMessage()
        assertTrue(messages.get(0).contains(details));
    }

    @Test
    void bookingConfirmationShouldContainSuccessfullyBooked() {
        notificationManager.sendBookingConfirmation("a@b.com", "details");
        List<String> messages = mockService.getSentMessages();
        assertTrue(messages.get(0).contains("successfully booked"));
    }

    // ── sendCancellationNotice ────────────────────────────────────────────────

    @Test
    void shouldSendCancellationNotice() {
        String email = "test@example.com";
        String details = "Appointment on 2025-01-15 at 10:00";

        notificationManager.sendCancellationNotice(email, details);

        List<String> messages = mockService.getSentMessages();
        assertEquals(1, messages.size());
        assertTrue(messages.get(0).contains("CANCELLATION"));
        assertTrue(messages.get(0).contains(email));
        assertTrue(messages.get(0).contains(details));
    }

    @Test
    void cancellationNoticeShouldContainCancelledPhrase() {
        notificationManager.sendCancellationNotice("a@b.com", "details");
        List<String> messages = mockService.getSentMessages();
        assertTrue(messages.get(0).contains("cancelled"));
    }

    // ── sendModificationNotice ────────────────────────────────────────────────

    @Test
    void shouldSendModificationNotice() {
        String email = "test@example.com";
        String oldDetails = "Old appointment: 2025-01-15 10:00";
        String newDetails = "New appointment: 2025-01-16 14:00";

        notificationManager.sendModificationNotice(email, oldDetails, newDetails);

        List<String> messages = mockService.getSentMessages();
        assertEquals(1, messages.size());
        // sendModificationNotice uses sendReminder → logged as REMINDER
        assertTrue(messages.get(0).contains("REMINDER"));
        assertTrue(messages.get(0).contains("modified"));
        assertTrue(messages.get(0).contains(oldDetails));
        assertTrue(messages.get(0).contains(newDetails));
    }

    @Test
    void modificationNoticeShouldBeLoggedAsReminder() {
        notificationManager.sendModificationNotice("a@b.com", "old", "new");
        List<String> messages = mockService.getSentMessages();
        assertEquals(1, messages.size());
        assertTrue(messages.get(0).startsWith("REMINDER"));
    }

    // ── MockNotificationService helpers ──────────────────────────────────────

    @Test
    void shouldClearMessages() {
        notificationManager.sendBookingConfirmation("test@test.com", "test");
        assertEquals(1, mockService.getSentMessages().size());

        mockService.clearMessages();

        assertEquals(0, mockService.getSentMessages().size());
    }

    @Test
    void getSentMessagesShouldReturnDefensiveCopy() {
        notificationManager.sendBookingConfirmation("a@b.com", "msg");
        List<String> copy = mockService.getSentMessages();
        copy.clear(); // mutate the returned copy
        assertEquals(1, mockService.getSentMessages().size()); // original unchanged
    }

    @Test
    void multipleCallsShouldAccumulateMessages() {
        notificationManager.sendBookingConfirmation("a@b.com", "b1");
        notificationManager.sendCancellationNotice("a@b.com", "c1");
        notificationManager.sendModificationNotice("a@b.com", "old", "new");
        assertEquals(3, mockService.getSentMessages().size());
    }

    @AfterEach
    void cleanup() {
        mockService.clearMessages();
    }
}
